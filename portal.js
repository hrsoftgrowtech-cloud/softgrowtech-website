...
async function initRegister(){
  ...
  const syncCountry=()=>{const opt=code?.selectedOptions?.[0],c=opt?.dataset?.country,iso=opt?.dataset?.iso,flag=document.getElementById('callingFlag'),selected=Boolean(code?.value);if(countryEl)countryEl.value=c||'';if(flag)flag.innerHTML=iso?`<img src="https://flagcdn.com/w40/${iso}.png" width="24" height="18" alt="${esc(c||'Country')}" loading="eager" referrerpolicy="no-referrer" style="display:block;object-fit:cover;border-radius:2px">`:'<span class="calling-flag-placeholder">🌐</span>';if(phoneEl){phoneEl.readOnly=!selected;phoneEl.placeholder=selected?'Mobile number':'Click to select calling code first';if(!selected){phoneEl.value='';phoneEl.setCustomValidity('');}else{phoneEl.setCustomValidity('');}}const help=document.getElementById('phoneHelp');if(help){help.hidden=true;help.textContent='';}};
  ...
  const showCallingCodePrompt=()=>{if(code.value)return;toast('Please select your country calling code first.');phoneEl.value='';phoneEl.setCustomValidity('');};phoneEl.addEventListener('click',()=>{if(!code.value)showCallingCodePrompt();});phoneEl.addEventListener('keydown',e=>{if(!code.value){e.preventDefault();showCallingCodePrompt();}});phoneEl.addEventListener('input',()=>{if(!code.value){phoneEl.value='';showCallingCodePrompt();return}phoneEl.setCustomValidity('')});phoneEl.addEventListener('blur',()=>{if(!code.value){phoneEl.setCustomValidity('');return}const v=validatePhone();phoneEl.setCustomValidity(v.ok?'':v.msg)});code.addEventListener('change',()=>{syncCountry();phoneEl.setCustomValidity('');});
  f.onsubmit=async e=>{e.preventDefault();const b=f.querySelector('button');b.disabled=true;b.textContent='Creating registration…';if(regError){regError.hidden=true;regError.textContent=''}try{
    ...
    phoneEl.setCustomValidity(phoneCheck.ok?'':phoneCheck.msg);if(!phoneCheck.ok)throw Error(phoneCheck.msg);
