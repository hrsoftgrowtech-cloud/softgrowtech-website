async function initRegister(){
  ...
  const showPhoneError=(msg='')=>{if(phoneHelp){phoneHelp.textContent=msg;phoneHelp.style.display=msg?'block':'none'}};
  const syncCountry=()=>{const opt=code?.selectedOptions?.[0],c=opt?.dataset?.country||'',iso=opt?.dataset?.iso||'',flag=document.getElementById('callingFlag'),selected=Boolean(code?.value);if(countryEl)countryEl.value=c;if(flag)flag.innerHTML=iso?`<img src="https://flagcdn.com/w40/${iso}.png" width="26" height="18" alt="${esc(c)}" loading="eager" referrerpolicy="no-referrer">`:'<span class="calling-flag-placeholder">🌐</span>';if(phoneEl){phoneEl.disabled=!selected;phoneEl.placeholder=selected?'Enter mobile number':'Select country code first';if(!selected){phoneEl.value='';showPhoneError('')}else{showPhoneError('');setTimeout(()=>phoneEl.focus(),0)}}};
  ...
  const validatePhone=()=>{const opt=code.selectedOptions?.[0],iso=opt?.dataset?.iso||'',cc=(code.value||'').replace('+',''),raw=phoneEl.value.trim();if(!cc||!iso)return{ok:false,msg:'Please select your country calling code first.'};if(!raw)return{ok:false,msg:'Please enter your mobile number.'};if(/[a-z]/i.test(raw))return{ok:false,msg:'Please enter a valid mobile number.'};const phoneLib=window.libphonenumber;if(phoneLib?.parsePhoneNumberFromString){try{const parsed=phoneLib.parsePhoneNumberFromString(raw,iso.toUpperCase());if(!parsed||!parsed.isValid()||parsed.country!==iso.toUpperCase()||parsed.countryCallingCode!==cc)return{ok:false,msg:`The mobile number does not match the selected country (${opt.dataset.country}).`};return{ok:true,digits:parsed.nationalNumber}}catch(_){}}let digits=raw.replace(/\D/g,'');if(raw.startsWith('+')||raw.startsWith('00')){const normalized=raw.startsWith('00')?raw.slice(2):raw.slice(1),foreign=allCodes.find(x=>normalized.startsWith(x));if(foreign&&foreign!==cc)return{ok:false,msg:`The mobile number does not match the selected country (${opt.dataset.country}).`};if(normalized.startsWith(cc))digits=normalized.slice(cc.length)}digits=digits.replace(/^0+/,'');const rule=countryRules[iso];if(!rule)return{ok:digits.length>=6&&digits.length<=15,msg:digits.length>=6&&digits.length<=15?'':'Please enter a valid mobile number.'};if(digits.length<rule.min||digits.length>rule.max||rule.starts&&!rule.starts.test(digits))return{ok:false,msg:`Please enter a valid ${opt.dataset.country} mobile number for ${code.value}.`};return{ok:true,digits}};
  phoneEl.addEventListener('input',()=>{phoneEl.value=phoneEl.value.replace(/\D/g,'');showPhoneError('')});
  phoneEl.addEventListener('blur',()=>{if(phoneEl.disabled)return;const v=validatePhone();if(!v.ok)showPhoneError(v.msg)});
  code.addEventListener('change',()=>{syncCountry();update()});
  f.onsubmit=async e=>{e.preventDefault();const b=f.querySelector('button');b.disabled=true;b.textContent='Creating registration…';showPhoneError('');if(regError){regError.hidden=true;regError.textContent=''}try{
    ...
    if(!callingCode){showPhoneError('Please select your country calling code first.');code.focus();throw Error('PHONE_VALIDATION')}
    if(!phoneCheck.ok){showPhoneError(phoneCheck.msg);phoneEl.focus();throw Error('PHONE_VALIDATION')}
  ...
  }catch(err){if(err?.message==='PHONE_VALIDATION'){b.disabled=false;b.textContent='Create Registration →';return}if(regError&&/already|registered|exists/i.test(err.message||'')){regError.innerHTML='<strong>Registration Already Exists</strong><br>This Gmail address is already registered with SoftGrowTech. Please use your existing registration instead of creating a new one.';regError.hidden=false}else{if(regError){regError.textContent=err.message||'Registration failed.';regError.hidden=false}else toast(err.message||'Registration failed.')}b.disabled=false;b.textContent='Create Registration →'}}
}
