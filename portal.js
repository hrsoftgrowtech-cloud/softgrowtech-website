...
async function initPortal(){
  ...
  const programStart=p.batch_start||null, programEnd=programStart?monthEnd(programStart):null;
  const {data:batchRow}=programStart?await sb.from('sgt_batches').select('start_date,orientation_date,orientation_status').eq('start_date',programStart).maybeSingle():{data:null};
  const orient=batchRow?.orientation_date||null;
  const orientationComplete=batchRow?.orientation_status==='complete';
  ...
  document.getElementById('schedule').innerHTML=scheduleItems.map(([name,start,end])=>{const state=name==='Orientation Session'&&orientationComplete?'complete':rangeState(start,end);const status=scheduleStatus(name,state);return `<div class="timeline-item"><span class="timeline-dot ${state}"></span><div style="flex:1"><strong>${name}</strong><small>${start?rangeLabel(start,end):'Coming Soon'}</small><div style="margin-top:7px"><span class="status ${state==='complete'?'success':state==='current'?'blue':'warn'}">${status}</span></div></div></div>`}).join('');

  document.getElementById('statusCards').innerHTML=[['Internship Program',programStatus],['Orientation',orientationComplete?'Orientation Complete':(orient?`Scheduled • ${dateFmt(orient)}`:'Coming Soon')],['Enroll Fee',p.payment_status],['Selection Assessment',p.assessment_status],['Selection',p.selection_status],['Review',p.review_status]].map(x=>`<div class="meta-box"><small>${x[0]}</small><span class="status ${statusClass(x[1])}">${esc(x[1])}</span></div>`).join('');

  const {data:grp}=await sb.from('sgt_settings').select('value').eq('key','registration_group').maybeSingle();
  const registrationGroupFromAuth=u.user_metadata?.whatsapp_group_url||'';const groupUrl=p.whatsapp_group_url||registrationGroupFromAuth||(!p.batch_start?(grp?.value?.url||''):'');
  ...
  if(groupAction) groupAction.innerHTML=`<div class="portal-card" style="margin-top:18px;border-color:#dbeafe;background:linear-gradient(180deg,#f8fbff,#fff)"><div class="portal-section-title"><div><div class="portal-kicker">Official Batch Group</div><h2 style="margin-bottom:4px">${esc(groupLabel)}</h2><p style="margin:0">Important updates, schedules, tasks and program information will be shared there. Joining the group is required.</p></div></div>${groupUrl?`<div class="portal-actions"><a class="portal-btn primary" href="${esc(groupUrl)}" target="_blank" rel="noopener">Join / Open →</a></div>`:'<div class="help-note" style="margin-top:12px">Your batch group link will appear here once it is assigned.</div>'}</div>`;
...
async function initAssessment(){
  ...
  const orientationDate=orient;const orientationReady=orientationComplete;
