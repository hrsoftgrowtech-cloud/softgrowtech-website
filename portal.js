...
async function initRegister(){
  ...
  const syncCountry=()=>{const opt=code?.selectedOptions?.[0],c=opt?.dataset?.country,iso=opt?.dataset?.iso,flag=document.getElementById('callingFlag');if(countryEl)countryEl.value=c||'';if(flag)flag.innerHTML=iso?`<img src="https://flagcdn.com/w40/${iso}.png" width="24" height="18" alt="${esc(c||'Country')}" loading="eager" referrerpolicy="no-referrer" style="display:block;object-fit:cover;border-radius:2px">`:'<span class="calling-flag-placeholder">🌐</span>'};
  ...
  phoneEl.addEventListener('blur',()=>{const v=validatePhone();phoneEl.setCustomValidity(v.ok?'':v.msg)});code.addEventListener('change',()=>phoneEl.setCustomValidity(''));
