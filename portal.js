...
async function initRegister(){
  ...
  f.onsubmit=async e=>{e.preventDefault();const b=f.querySelector('button');b.disabled=true;b.textContent='Creating registration…';if(regError){regError.hidden=true;regError.textContent=''}try{
    ...
    const referral_code=getCapturedReferralCode();const {error}=await sb.auth.signUp({email,password:temp,options:{emailRedirectTo:location.origin+'/email-confirmed.html',data:{full_name:name,phone,country,calling_code:callingCode,study_year,gender,domain,address,temp_password:temp,must_change_password:true,whatsapp_group_url,referral_code}}});
...
async function initPortal(){
  ...
  const displayAssessmentStatus=p.selection_status==='Not Selected'?'Not Selected':p.assessment_status;document.getElementById('statusCards').innerHTML=[['Internship Program',programStatus],['Enroll Fee',p.payment_status],['Selection Assessment',displayAssessmentStatus],['Selection',p.selection_status],['Review',p.review_status]].map(x=>`<div class="meta-box"><small>${x[0]}</small><span class="status ${statusClass(x[1])}">${esc(x[1])}</span></div>`).join('');

      const registrationGroupFromAuth=u.user_metadata?.whatsapp_group_url||'';const groupUrl=p.whatsapp_group_url||registrationGroupFromAuth||'';
  ...
  if(paymentRejected){const remaining=formatRemaining(sevenDayDeadline);action=`<div class="help-note"><strong>Payment Rejected</strong><p>Your nominal Enrollment Fee payment was rejected. Please submit a valid payment within the available time.</p><div style="font-weight:800;margin:10px 0">${esc(remaining)}</div></div><div class="portal-actions"><a class="portal-btn primary" href="enrollment.html">Complete Nominal Enrollment Fee →</a></div>`;}else if(p.selection_status==='Not Selected'){
    if(round>=2){
    ...
    const completed=(p.status==='Completed'||(programEnd&&today>escDate(programEnd)));
    if(completed){
      action=`<div class="success-panel"><strong class="icon-inline">${window.sgtIcon?window.sgtIcon('trophy'):''}Internship Completed</strong><p style="margin:6px 0 0">Congratulations on successfully completing your SoftGrowTech Internship Program! Your internship journey has reached an important milestone. Keep learning, keep building and keep growing.</p></div>`;
    }else if(programStart&&today>=escDate(programStart)){
      action=`<div class="success-panel"><strong class="icon-inline">${window.sgtIcon?window.sgtIcon('tasks'):''}Your Internship Has Started</strong><p style="margin:6px 0 0">Your SoftGrowTech internship is now underway. Stay consistent, complete your assigned tasks, and make the most of your learning journey.</p></div>`;
    }else{
      const batchText=programStart?` Your internship journey will begin with your assigned batch${programStart?` on ${dateFmt(programStart)}`:''}.`:'';
      action=`<div class="success-panel"><strong class="icon-inline">${window.sgtIcon?window.sgtIcon('check'):''}You're All Set!</strong><p style="margin:6px 0 0">Your assessment and enrollment process are complete. You are officially selected for the SoftGrowTech Internship Program.${esc(batchText)}</p></div>`;
    }
  ...
  if(!projectAccess){
    ...
    const renderStage=(title,description,start,end,kind,extra='')=>{
      ...
      } else if(isClosed){status='Closed';dot='complete';actions=`<a class="portal-btn secondary icon-btn" href="https://wa.me/917839686310" target="_blank" rel="noopener">${window.sgtIcon?window.sgtIcon('support'):''}Couldn’t submit on time? Get Help</a>`}
  ...
  const openNotificationPanel=async()=>{
    ...
    panel.innerHTML=`<div class="student-notification-panel-head"><div><strong>Notifications</strong><small>All caught up</small></div><button class="close-btn" type="button" aria-label="Close notifications">×</button></div><div class="student-notification-list">${notificationRows.length?notificationRows.map(x=>`<button type="button" class="student-notification-item read" data-note-id="${esc(x.id)}"><span class="note-icon">${window.sgtIcon?window.sgtIcon('bell'):''}</span><span><strong>${esc(x.category)}</strong><small style="white-space:pre-line">${esc(x.message)}</small><em>${adt(x.created_at)} • Seen</em></span></button>`).join(''):'<div class="admin-note-empty">No notifications yet.</div>'}</div>`;
...
async function initAssessment(){
  ...
  const currentAttemptNo=set==='reassessment-v2'?2:1;const {data:existingAttempt}=await sb.from('sgt_assessment_attempts').select('id,status,submitted_at').eq('user_id',u.id).eq('attempt_no',currentAttemptNo).maybeSingle();if(existingAttempt?.status==='Complete'){const re=set==='reassessment-v2';root.innerHTML=`<div class="portal-card"><div class="portal-kicker">${re?'Re-Assessment':'Selection Assessment'}</div><h1>Assessment Already Submitted</h1><div class="success-panel"><strong>Your ${re?'re-assessment':'selection assessment'} has already been submitted and is currently under review. Please wait for the result.</strong><p style="margin:8px 0 0">Status: Under Review</p></div><div class="portal-actions"><a class="portal-btn secondary" href="portal.html">Back to Dashboard</a></div></div>`;return}
  ...
  const submitAssessment=async(auto=false)=>{
    ...
    const finalAttemptNo=set==='reassessment-v2'?2:1;const {data:already}=await sb.from('sgt_assessment_attempts').select('id,status').eq('user_id',u.id).eq('attempt_no',finalAttemptNo).maybeSingle();if(already?.status==='Complete'){toast('This assessment has already been submitted.');location.href='portal.html';return}const submittedNow=new Date().toISOString();const {data:a,error}=await sb.from('sgt_assessment_attempts').insert({student_id:p.student_id,user_id:u.id,attempt_no:finalAttemptNo,question_set:set,status:'Complete',submitted_at:submittedNow}).select().single();
    ...
    const profileUpdate=await sb.from('sgt_profiles').update({assessment_status:'Complete',review_status:'Under Review',assessment_completed_at:submittedNow,updated_at:new Date().toISOString()}).eq('id',u.id);
...
async function initEnrollment(){
  ...
  const {data:s}=await sb.from('sgt_settings').select('value').eq('key','payment').maybeSingle();const cfg=s?.value||{};const methods=[];
  const first=(...v)=>v.find(x=>x!==undefined&&x!==null&&String(x).trim()!=='');
  const normalizeMethod=(x={})=>({enabled:x.enabled!==false,fee:first(x.fee,x.amount,x.price,''),currency:first(x.currency,x.currency_code,''),details:first(x.details,x.payment_details,x.paymentDetails,x.account_details,x.accountDetails,x.instructions,''),qr_url:first(x.qr_url,x.qr,x.qrCode,x.qr_code,'')});
  const country=String(p.country||'').trim().toLowerCase(),phone=String(p.phone||'').replace(/\s+/g,'');const codeMatch=phone.match(/^\+(\d{1,4})/),code=codeMatch?.[1]||'';const india=code==='91'||['india','in','bharat'].includes(country),pakistan=code==='92'||['pakistan','pk'].includes(country);const defaultQr={india:'',binance:'',paypal:'',pakistan:'',wise:''};const qrFor=(key,value)=>value||defaultQr[key]||'';
  ...
  if(india){const x=normalizeMethod(cfg.india||{});if(x.enabled)methods.push({key:'india',name:'India — UPI',fee:`${x.fee||'149'} ${x.currency||'INR'}`,details:x.details||'UPI ID: info.softgrowtech@oksbi',qr:qrFor('india',x.qr_url)})}
  else if(pakistan){const x=normalizeMethod(cfg.pakistan||{});if(x.enabled)methods.push({key:'pakistan',name:'Pakistan — JazzCash / Easypaisa',fee:`${x.fee||'499'} ${x.currency||'PKR'}`,details:x.details||'Payment details are configured by SoftGrowTech.',qr:qrFor('pakistan',x.qr_url)})}
  else{const intl=cfg.international||{};['binance','paypal','wise'].forEach(k=>{const x=normalizeMethod(intl[k]||{});if(x.enabled)methods.push({key:k,name:k==='binance'?'Binance':k==='paypal'?'PayPal':'Wise',fee:`${x.fee||intl.fee||'5'} ${x.currency||intl.currency||'USD'}`,details:x.details||'Payment details are configured by SoftGrowTech.',qr:qrFor(k,x.qr_url)})})}
