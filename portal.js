...
async function initRegister(){
  ...
  const syncCountry=()=>{const opt=code?.selectedOptions?.[0],c=opt?.dataset?.country,iso=opt?.dataset?.iso,flag=document.getElementById('callingFlag'),selected=Boolean(code?.value);if(countryEl)countryEl.value=c||'';if(flag)flag.innerHTML=iso?`<img src="https://flagcdn.com/w40/${iso}.png" width="24" height="18" alt="${esc(c||'Country')}" loading="eager" referrerpolicy="no-referrer" style="display:block;object-fit:cover;border-radius:2px">`:'<span class="calling-flag-placeholder">🌐</span>';if(phoneEl){phoneEl.disabled=!selected;phoneEl.placeholder=selected?'Mobile number':'Select calling code first';if(!selected){phoneEl.value='';phoneEl.setCustomValidity('Please select your country calling code first.');}else{phoneEl.setCustomValidity('');}}const help=document.getElementById('phoneHelp');if(help)help.textContent=selected?`Enter a ${c||'valid'} mobile number for ${code.value}.`:'Please select your country calling code first.'};
  ...
  phoneEl.addEventListener('input',()=>{if(!code.value){phoneEl.value='';phoneEl.setCustomValidity('Please select your country calling code first.');return}phoneEl.setCustomValidity('')});phoneEl.addEventListener('blur',()=>{const v=validatePhone();phoneEl.setCustomValidity(v.ok?'':v.msg)});code.addEventListener('change',()=>{syncCountry();phoneEl.setCustomValidity(code.value?'':'Please select your country calling code first.')});
