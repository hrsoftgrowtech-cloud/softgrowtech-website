...
async function initPortal(){
  ...
  if(paymentRejected){const remaining=formatRemaining(sevenDayDeadline);action=`<div class="help-note"><strong>Payment Rejected</strong><p>Your enrollment fee payment was rejected. Please submit a valid payment within the available time.</p><div style="font-weight:800;margin:10px 0">${esc(remaining)}</div></div><div class="portal-actions"><a class="portal-btn primary" href="enrollment.html">Complete Enrollment Fee →</a></div>`;}else if(p.selection_status==='Not Selected'){
    ...
    const selectedMessage=p.payment_status==='Verified'?'Your assessment has been reviewed and your enrollment fee has been verified. Continue your program and get ready for your batch start.':p.payment_status==='Under Verification'?'Your assessment has been reviewed and you have been selected. Your enrollment fee payment is currently under verification. Please wait for payment verification.':'Your assessment has been reviewed and you have been selected. Please check your payment status and continue from your dashboard.';
    action=`<div class="success-panel"><strong>Selection Confirmed ✓</strong><p style="margin:6px 0 0">${esc(selectedMessage)}</p></div>`;
