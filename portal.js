...
async function initAssessment(){
  ...
  const submitAssessment=async(auto=false)=>{
    ...
    const {data:a,error}=await sb.from('sgt_assessment_attempts').insert({student_id:p.student_id,user_id:u.id,attempt_no:2,question_set:set,status:'Complete',submitted_at:new Date().toISOString()}).select().single();
