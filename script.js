...
function initResultPage() {
  ...
  const VERIFICATION_BACK_URL = "documents-verification.html";
    ...
    root.innerHTML = `
      <div class="invalid-only-page">
        <div class="invalid-result">
          ...
          <a class="result-button" href="${VERIFICATION_BACK_URL}">Verify Another ID <span>→</span></a>
        </div>
      </div>`;
  };
    ...
    root.innerHTML = `
      <div class="invalid-only-page">
        <div class="invalid-result">
          ...
          <a class="result-button" href="${VERIFICATION_BACK_URL}">Try Again <span>→</span></a>
        </div>
      </div>`;
  });
}
    ...
    <div class="result-shell softgrow-result-animate">
      ...
      <div class="result-main">
        ...
        <div class="result-actions">
          ...
          <a class="result-button verify-another" href="${VERIFICATION_BACK_URL}">Verify Another Letter ID <span>→</span></a>
        </div>
...
function downloadVerificationRecord() {
  ...
  Promise.all([
    ...
    // Clone the COMPLETE visible result page.
    // Remove only the two action controls and their helper line.
    const page = document.createElement("div");
    page.style.cssText =
      "position:fixed;left:-100000px;top:0;width:" +
      Math.max(document.documentElement.clientWidth, shell.offsetWidth) +
      "px;background:#fff;margin:0;padding:0;";
    ...
    page.appendChild(shellClone);

    // The trust/support strip is part of the downloadable record.
    if (trust) {
      page.appendChild(trust.cloneNode(true));
    }

    document.body.appendChild(page);

    // Allow cloned fonts/layout to settle before capture.
    ...
    // Keep the working page available even if the external PDF libraries fail.
    window.print();
  });
}
